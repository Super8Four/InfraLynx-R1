...
import type { Site, Location, RackRole, RackType, TenantGroup, Tenant, Rack, Device, DeviceType } from "@prisma/client"

type ProcessedRack = Rack & {
  site: { name: string } | null;
  location: { name: string } | null;
  tenant: { name: string } | null;
  role: { name: string } | null;
  type: { model: string } | null;
  devices: (Device & { deviceType: DeviceType | null; })[];
  deviceCount: number;
  spaceUtilization: number;
};


interface RacksClientPageProps {
  initialRacks: ProcessedRack[];
  sites: Site[];
  locations: Location[];
  rackRoles: RackRole[];
  rackTypes: RackType[];
...
const rackSchema = z.object({
  ...
  status: z.enum(["active", "planned", "decommissioning"]),
...
export function RacksClientPage({
  initialRacks,
  sites,
  locations,
  rackRoles,
  ...
  tenantGroups,
  tenants
}: RacksClientPageProps) {
  const { toast } = useToast()
  const [racks, setRacks] = useState<ProcessedRack[]>(initialRacks)
  ...
  // TODO: Add createRack server action
  ...
  const handleDelete = (id: string) => {
    // TODO: Add deleteRack server action
                        ...
                        <DialogContent className="sm:max-w-4xl">
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)}>
                                ...
                                <div className="space-y-8">
                                    ...
                                    <FormField control={form.control} name="comments" render={({ field }) => ( <FormItem className="mt-4"> <FormLabel>Comments</FormLabel> <FormControl><Textarea placeholder="A brief description of the rack." {...field} /></FormControl> <FormMessage /> </FormItem> )} />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                                        ...
                                        <FormItem><FormLabel>Width</FormLabel>
                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                            <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                                            <SelectContent><SelectItem value="nineteen_in">19 inches</SelectItem><SelectItem value="twentythree_in">23 inches</SelectItem></SelectContent>
                                            </Select><FormMessage />
                                        </FormItem>
                                        )}/>
            ...
            <TableBody>
              {racks.map((rack) => (
                <TableRow key={rack.id}>
                    ...
                    <TableCell className="hidden sm:table-cell">{rack.site?.name ?? "—"}</TableCell>
                    <TableCell className="hidden lg:table-cell">{rack.location?.name ?? '—'}</TableCell>
                    <TableCell>{getStatusBadge(rack.status)}</TableCell>
                    <TableCell className="hidden lg:table-cell">{rack.facilityId || '—'}</TableCell>
                    <TableCell className="hidden lg:table-cell">{rack.tenant?.name ?? '—'}</TableCell>
                    <TableCell className="hidden md:table-cell">
                        {rack.role?.name ? <Badge variant="secondary">{rack.role.name}</Badge> : '—'}
                    </TableCell>
                    <TableCell className="hidden lg:table-cell">{rack.type?.model ?? '—'}</TableCell>
