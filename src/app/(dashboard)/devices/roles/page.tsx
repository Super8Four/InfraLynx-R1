...
import { initialDeviceRoles, type DeviceRole } from "@/lib/data"
...
export default function DeviceRolesPage() {
  ...
  const [roles, setRoles] = useState<DeviceRole[]>(initialDeviceRoles)
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false)

  const form = useForm<RoleFormValues>({
    resolver: zodResolver(roleSchema),
    defaultValues: { name: "", description: "", color: "bg-gray-500" },
  })
  
  function onSubmit(data: RoleFormValues) {
    const newRole: DeviceRole = {
      id: `d-role-${Date.now()}`,
      ...data,
              ...
              <DialogContent>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    />
                    {/* A proper color picker would be better here in a real app */}
                     <FormField
                      control={form.control}
                      name="color"
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g., bg-blue-500" {...field} />
                          </FormControl>
                           <FormMessage />
                        </FormItem>
                      )}
