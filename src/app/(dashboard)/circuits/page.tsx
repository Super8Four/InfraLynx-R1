...
export default function CircuitsPage() {
  ...
  const [circuitTypes] = useState(initialCircuitTypes)
  ...
  function onSubmit(data: CircuitFormValues) {
    const newCircuit: Circuit = {
      id: `circ-${Date.now()}`,
      ...data,
      installDate: data.installDate || new Date().toISOString().split('T')[0],
      commitRate: data.commitRate || 0,
  ...
  const getTypeName = (id: string) => circuitTypes.find(t => t.id === id)?.name ?? 'Unknown'
